...
class _HalamanHasilQuizState extends State<HalamanHasilQuiz> {
  ...
  double get _average => _studentResults.isEmpty 
      ? 0 
      : _studentResults.fold<int>(0, (sum, s) => sum + (s['score'] as int)) / _studentResults.length;

  int get _passedCount => _studentResults.where((s) => (s['score'] as int) >= 75).length;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon: const Icon(Icons.file_download_outlined, color: AppColors.textWhite),
            onPressed: () {},
          ),
        ],
      ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(widget.quizTitle, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                const SizedBox(height: 4),
                Text(widget.quizSubject, style: TextStyle(fontSize: 13, color: Colors.grey.shade600)),
              ],
            ),
          ),
        ],
  ...
  Widget _buildStatistics() {
    return Row(
      children: [
        Expanded(child: _buildStatCard('Rata-rata', _average.toStringAsFixed(1), Icons.trending_up, Colors.blue)),
        const SizedBox(width: 12),
        Expanded(child: _buildStatCard('Pengumpulan', '${_studentResults.length}', Icons.people_outline, Colors.green)),
        const SizedBox(width: 12),
        Expanded(child: _buildStatCard('Lulus', '$_passedCount', Icons.check_circle_outline, Colors.orange)),
      ],
    );
  }
          ...
          Row(
            children: [
              CircleAvatar(
                ...
                child: Text(student['avatar'], style: TextStyle(color: scoreColor, fontWeight: FontWeight.bold)),
              ),
              const SizedBox(width: 12),
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('NIS: ${student['studentId']}', style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                  ],
                ),
              ),
              Container(
                ...
                child: Text('${student['score']}', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: scoreColor)),
              ),
            ],
          ),
                ...
                Expanded(
                  child: Row(
                    children: [
                      ...
                      Text(student['duration'], style: TextStyle(fontSize: 12, color: Colors.grey.shade700)),
                    ],
                  ),
                ),
                Expanded(
